...
//Algorithms:
//Graham's Scan 
//maintains a stack S of candidate points. The points that are not
//vertices of CH(Q) are eventually popped from the stack.
//When the algorithm terminates, stack S contains exactly the 
//vertices of CH(Q), in counterclockwise order.
//
// Divide_and_conquer
// using parallelization to improve program efficiency
// Divides a deque of all points by two. 
// if the desired number of process is bigger than 1,
// fork a child process.
// The child process recursively call divide_and_conquer to deal
// with half set of points.
// until created processNum-1 process, keep checking each subset,
// recursively call divide_and_conquer method to keep dividing 
// subset until it has only one point. 
// 
// Then, merges and pass valid subset back to the method on stack.
// child process set the subset back to the parent process through
// pipe. 
//
//Assumptions: 
//Assume this program is going to run on a mahine with four or
//more than 4 cores.
//
// ---------------------------------------------------------------- 
#include <iostream>
...
void recv(int fd, deque<Point> &q) {
	...
	for (int i = 0; i < size; i++) {
		...
		q.push_back(Point(x[i], y[i], id[i]));
	}
}
...
// --------------------- divide_and_conquer------------------------ 
// Description: 
// Divides a deque of all poitns by two. 
// if the processNum passes from parameter is bigger than 1,
// creating a child process.
// Child process copy all the data and instructions from parent 
// process, then recursively call divide_and_conquer to deal with
// half set of points.
// until created processNum-1 process, keep checking each subset,
// recursively call divide_and_conquer method to keep dividing 
// subset until it has only one point. 
// 
// therefor, merges and use Graham algorithm to check each points
// and pass the valid subset back to the method on stack.
// child process set the subset back to the parent process through
// pipe. 
// in this way, using parallelization to improve program efficiency
// ---------------------------------------------------------------- 

void divide_and_conquer(deque<Point> &q, int processNum)
{
	
	deque<Point> s1, s2, s2Recv;
	...
	if (processNum > 1)
	{
		...
		{
			if (s2.size() > 1)//  right queue
			{
				divide_and_conquer(s2, processNum / 2);
				close(pipefd[RD]);
				send(pipefd[WR], s2);
				exit(0);
			}
			...
			if (s1.size() > 1)//  left queue
			{
				divide_and_conquer(s1, processNum / 2);
				close(pipefd[WR]);
				wait(NULL);
		...
		if (s1.size() > 1)    
			divide_and_conquer(s1, processNum);  // left queue
		if (s2.size() > 1)    
			divide_and_conquer(s2, processNum);  // right queue
		merge(q, s1, s2);          // merge them
		graham(q);                 // apply the graham algorithm
	}
}
...
int main(int argc, char *argv[]) {
	...
	int secondParameter = atoi(argv[2]);

	// convex hull
	divide_and_conquer(points, secondParameter);
